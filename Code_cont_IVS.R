...
gen_data=function(n=200,n2=50,rho=0.6,cormx=0.4,TE=log(2),MP=0.5,PY=0.5,outcome="binary") {
  ...
  if (outcome=="binary") {
    beta0.f=function(beta0) {
      ...
      z=spatstat::gauss.hermite(f=function(x) {exp(x)}, mu = m.f, sd = sqrt(v.f[1,1]),  order = 40)
      PY-z
    }
...
get_eee1=function(data,Xname="X",Xsname="Xs",Yname="Y",Mname="M",Wname="W",Zname="W",Iname="I") {
  ...
  gradU=function(name="Ua",a,sa,b,sb,c,sc,d,sd,model=1) {
    ...
    if (model==1) {
      x=c(a,sa,c,sc);loc1=1:length(a);loc2=length(a)+1;
      loc3=(length(a)+2):(length(a)+1+length(c));loc4=length(x)
      myf=function(x,data.main,data.cali) {
        get(name)(a=x[loc1],sa=x[loc2],c=x[loc3],sc=x[loc4],data.main=data.main,data.cali=data.cali)
      }
    } else {
      x=c(b,sb,d,sd);loc1=1:length(b);loc2=length(b)+1;
      loc3=(length(b)+2):(length(b)+1+length(d));loc4=length(x)
      myf=function(x,data.main,data.cali) {
        get(name)(b=x[loc1],sb=x[loc2],d=x[loc3],sd=x[loc4],data.main=data.main,data.cali=data.cali)
      }
    }
    mygrad(myf,x0=x,data.main=subset(mydata,I==0),data.cali=subset(mydata,I==1))
  }
  # Marginal Outcome Model
  m1=lm(fmar,data=mydata)
  ahat0 = m1$coefficients
  sigma2a = var(m1$residuals)
  # Conditional Outcome Model
  m2=lm(fcon,data=mydata)
  bhat0 = m2$coefficients
  sigma2b = var(m2$residuals)
  ...
  ahat[1] = ahat0[1] - ahat[2]*chat[1]
  # corrected b
  bhat=m2$coefficients
  bhat[2] = bhat0[2]/dhat[2]
  bhat[3] = bhat0[3] - bhat[2]*dhat[3]
  bhat[1] = bhat0[1] - bhat[2]*dhat[1]
  
  NIE.3s=  ahat[2]-bhat[2]
  MP.3s = (ahat[2]-bhat[2])/ahat[2]
  # ## mediation analysis with true X
  # m11=lm(Y~X,data=mydata)
  # at = m11$coefficients
  # # Conditional Outcome Model
  # m21=lm(Y~X+M,data=mydata)
  # bt = m21$coefficients
  # NIE.t = at[2]-bt[2]
  # MP.t = (at[2]-bt[2])/at[2]
  ...
  ############ Ua
  Ua=function(a,sa,c,sc,data.main,data.cali,type=1) {
    ...
    # conditional distribution X|Y,X* 
    mx=(sa*(c[1]+c[2]*data.main[,Xsname]+Zterm) - sc*a[2]*(a[1]+Wterm-data.main$Y))/(sa+a[2]^2 * sc)
    sx=(sa*sc)/(sa+a[2]^2 *sc)
    Ex2=sx+mx^2
    # main dataset
    out = array(0,dim=c(2+nw,n1+n2))
    out[1,1:n1]= 1* data.main[,Yname] - a[1] - a[2]*mx - Wterm 
    out[2,1:n1]= mx*data.main[,Yname] - a[1]*mx - a[2]*Ex2 - mx*Wterm
    out[1,(n1+1):(n1+n2)]=1*(data.cali[,Yname]-a[1]-a[2]*data.cali[,Xname]-Wterm_cali)
    out[2,(n1+1):(n1+n2)]=data.cali[,Xname]*(data.cali[,Yname]-a[1]-a[2]*data.cali[,Xname]-Wterm_cali)
    if (nw>0) {
      for (m in (1:nw)) {
        out[2+m,1:n1]= (1* data.main[,Yname] - a[1] - a[2]*mx - Wterm)*data.main[,Wname[m]]
        out[2+m,(n1+1):(n1+n2)]=data.cali[,Wname[m]]*(data.cali[,Yname]-a[1]-a[2]*data.cali[,Xname]-Wterm_cali)
      }
    }
  ...
  ########### Usa
  Usa=function(sa,a,c,sc,data.main,data.cali,type=1) {
    data.main=subset(mydata,mydata[,Iname]==0)
    data.cali = subset(mydata,mydata[,Iname]==1)
    n1=dim(data.main)[1];n2=dim(data.cali)[1]
    Wterm=`if`(nw==0,0, c(as.matrix(data.main[,Wname,drop=F]) %*% a[-c(1:2)]) )
    ...
    out[1,1:n1] = (data.main$Y-a[1]-Wterm)^2 - 2*(data.main$Y-a[1]-Wterm)*a[2]*mx + a[2]^2 * Ex2 - sa
    out[1,(n1+1):(n1+n2)]=(data.cali$Y-a[1]-a[2]*data.cali$X-Wterm_cali)^2 - sa
    if (type==2) {return(out)}
    apply(out,1,sum)
  }
  
  #### Uc
  Uc=function(c,a,sa,sc,data.main,data.cali,type=1) {
    data.main=subset(mydata,mydata[,Iname]==0)
    data.cali = subset(mydata,mydata[,Iname]==1)
    n1=dim(data.main)[1];n2=dim(data.cali)[1]
    Wterm=`if`(nw==0,0, c(as.matrix(data.main[,Wname,drop=F]) %*% a[-c(1:2)]) )
    ...
    out[1,1:n1]= 1 * (mx-c[1]-c[2]*data.main[,Xsname]-Zterm)
    out[2,1:n1]= data.main[,Xsname] * (mx-c[1]-c[2]*data.main[,Xsname]-Zterm)
    ...
    if (nz>0) {
      for (m in (1:nz)) {
        out[2+m,1:n1]= (mx-c[1]-c[2]*data.main[,Xsname]-Zterm)*data.main[,Zname[m]]
        out[2+m,(n1+1):(n1+n2)]=data.cali[,Zname[m]]*(data.cali[,Xname]-c[1]-c[2]*data.cali[,Xsname]-Zterm_cali)
      }
    }
    if (type==2) {return(out)}
    apply(out,1,sum)
  ...
  Usc=function(sc,a,c,sa,data.main,data.cali,type=1) {
    data.main=subset(mydata,mydata[,Iname]==0)
    data.cali = subset(mydata,mydata[,Iname]==1)
    n1=dim(data.main)[1];n2=dim(data.cali)[1]
    Wterm=`if`(nw==0,0, c(as.matrix(data.main[,Wname,drop=F]) %*% a[-c(1:2)]) )
    ...
    # main dataset
    out = array(0,dim=c(1,n1+n2))
    out[1,1:n1] = Ex2 - 2*(c[1]+c[2]*data.main[,Xsname]+Zterm)*mx + (c[1]+c[2]*data.main[,Xsname]+Zterm)^2 - sc
    out[1,(n1+1):(n1+n2)]=(data.cali[,Xname]-c[1]-c[2]*data.cali[,Xsname]-Zterm_cali)^2 - sc
    if (type==2) {return(out)}
    apply(out,1,sum)
  }
  ############ Ub
  Ub=function(b,sb,d,sd,data.main,data.cali,type=1) {
    data.main=subset(mydata,mydata[,Iname]==0)
    data.cali = subset(mydata,mydata[,Iname]==1)
    ...
    n1=dim(data.main)[1];n2=dim(data.cali)[1]
    # conditional distribution X|Y,X*,M
    mx=(sb*(d[1]+d[2]*data.main[,Xsname]+d[3]*data.main[,Mname]+Zterm) - sd*b[2]*(b[1]+b[3]*data.main[,Mname]+Wterm-data.main[,Yname]))/(sb+b[2]^2 * sd)
    sx=(sb*sd)/(sb+b[2]^2 *sd)
    Ex2=sx+mx^2
    # main dataset
    out = array(0,dim=c(3+nw,n1+n2))
    out[1,1:n1]= 1* (data.main[,Yname] - b[1] - b[2]*mx - b[3]*data.main[,Mname]-Wterm)
    out[2,1:n1]= mx*data.main[,Yname] - b[1]*mx - b[2]*Ex2 - b[3]*mx*data.main[,Mname] - mx*Wterm
    out[3,1:n1]= data.main[,Mname]* (data.main[,Yname] - b[1] - b[2]*mx - b[3]*data.main[,Mname]-Wterm)
    out[1,(n1+1):(n1+n2)]=1*(data.cali[,Yname]-b[1]-b[2]*data.cali[,Xname]-b[3]*data.cali[,Mname])
    out[2,(n1+1):(n1+n2)]=data.cali[,Xname]*(data.cali[,Yname]-b[1]-b[2]*data.cali[,Xname]-b[3]*data.cali[,Mname]-Wterm_cali)
    out[3,(n1+1):(n1+n2)]=data.cali[,Mname]*(data.cali[,Yname]-b[1]-b[2]*data.cali[,Xname]-b[3]*data.cali[,Mname]-Wterm_cali)
    if (nw>0) {
      for (m in (1:nw)) {
        out[3+m,1:n1]= data.main[,Wname[m]]*(data.main[,Yname] - b[1] - b[2]*mx - b[3]*data.main[,Mname]-Wterm)
        out[3+m,(n1+1):(n1+n2)]=data.cali[,Wname[m]]*(data.cali[,Yname]-b[1]-b[2]*data.cali[,Xname]-b[3]*data.cali[,Mname]-Wterm_cali)
      }
    }
    if (type==2) {return(out)}
    apply(out,1,sum)
  ...
  ########### Usb
  Usb=function(sb,b,d,sd,data.main,data.cali,type=1) {
    data.main=subset(mydata,mydata[,Iname]==0)
    data.cali = subset(mydata,mydata[,Iname]==1)
    Wterm=`if`(nw==0,0, c(as.matrix(data.main[,Wname,drop=F]) %*% b[-c(1:3)]) )
    Zterm=`if`(nz==0,0, c(as.matrix(data.main[,Zname,drop=F]) %*% d[-c(1:3)]) )
    ...
    # main dataset
    out = array(0,dim=c(1,n1+n2))
    out[1,1:n1] = (data.main[,Yname]-b[1]-b[3]*data.main[,Mname]-Wterm)^2 - 2*b[2]*mx*(data.main[,Yname]-b[1]-b[3]*data.main[,Mname]-Wterm) + b[2]^2 * Ex2 -sb
    out[1,(n1+1):(n1+n2)]=(data.cali[,Yname]-b[1]-b[2]*data.cali[,Xname]-b[3]*data.cali[,Mname]-Wterm_cali)^2 - sb
    if (type==2) {return(out)}
    apply(out,1,sum)
  }
  
  #### Ud
  Ud=function(d,b,sb,sd,data.main,data.cali,type=1) {
    data.main=subset(mydata,mydata[,Iname]==0)
    data.cali = subset(mydata,mydata[,Iname]==1)
    Wterm=`if`(nw==0,0, c(as.matrix(data.main[,Wname,drop=F]) %*% b[-c(1:3)]) )
    Zterm=`if`(nz==0,0, c(as.matrix(data.main[,Zname,drop=F]) %*% d[-c(1:3)]) )
    ...
    out[1,1:n1]= 1 * (mx-d[1]-d[2]*data.main[,Xsname]-d[3]*data.main[,Mname]-Zterm)
    out[2,1:n1]= data.main[,Xsname] * (mx-d[1]-d[2]*data.main[,Xsname]-d[3]*data.main[,Mname]-Zterm)
    out[3,1:n1]= data.main[,Mname] * (mx-d[1]-d[2]*data.main[,Xsname]-d[3]*data.main[,Mname]-Zterm)
    ...
    if (nz>0) {
      for (m in (1:nz)) {
        out[3+m,1:n1]= data.main[,Zname[m]]*(mx-d[1]-d[2]*data.main[,Xsname]-d[3]*data.main[,Mname]-Zterm)
  ...
  Usd=function(sd,b,d,sb,data.main,data.cali,type=1) {
    data.main=subset(mydata,mydata[,Iname]==0)
    data.cali = subset(mydata,mydata[,Iname]==1)
    Wterm=`if`(nw==0,0, c(as.matrix(data.main[,Wname,drop=F]) %*% b[-c(1:3)]) )
    Zterm=`if`(nz==0,0, c(as.matrix(data.main[,Zname,drop=F]) %*% d[-c(1:3)]) )
    ...
    # main dataset
    out = array(0,dim=c(1,n1+n2))
    out[1,1:n1] = Ex2 - 2*(d[1]+d[2]*data.main[,Xsname]+d[3]*data.main[,Mname]+Zterm)*mx + (d[1]+d[2]*data.main[,Xsname]+d[3]*data.main[,Mname]+Zterm)^2 - sd
  ...
  par.vec=c(ahat,sigma2a,chat,sigma2c)
  out = matrix(NA,ncol=length(par.vec),nrow=30)
  out[1,]=par.vec
  for (i in (2:30)) {
    ahat=nleqslv(x=ahat,fn=Ua,sa=sigma2a,c=chat,sc=sigma2c,
                 data.main=subset(mydata,mydata[,Iname]==0),data.cali=subset(mydata,mydata[,Iname]==1))$x
    sigma2a = nleqslv(x=sigma2a,fn=Usa,a=ahat,c=chat,sc=sigma2c,
                      data.main=subset(mydata,mydata[,Iname]==0),
                      data.cali=subset(mydata,mydata[,Iname]==1))$x
    chat=nleqslv(x=chat,fn=Uc,a=ahat,sc=sigma2c,sa=sigma2a,
                 data.main=subset(mydata,mydata[,Iname]==0),
                 data.cali=subset(mydata,mydata[,Iname]==1))$x
    sigma2c=nleqslv(x=sigma2c,fn=Usc,a=ahat,c=chat,sa=sigma2a,
                    data.main=subset(mydata,mydata[,Iname]==0),
                    data.cali=subset(mydata,mydata[,Iname]==1))$x
    par.vec=c(ahat,sigma2a,chat,sigma2c)
    out[i,]=par.vec
  ...
  names(par_mar)=c("a0","a1",aW,"sa","c0","c1",cZ,"sc")
  
  par.vec=c(bhat,sigma2b,dhat,sigma2d)
  out = matrix(NA,ncol=length(par.vec),nrow=30)
  out[1,]=par.vec
  for (i in (2:30)) {
    bhat=nleqslv(x=bhat,fn=Ub,sb=sigma2b,d=dhat,sd=sigma2d,
                 data.main=subset(mydata,mydata[,Iname]==0),data.cali=subset(mydata,mydata[,Iname]==1))$x
    sigma2b = nleqslv(x=sigma2b,fn=Usb,b=bhat,d=dhat,sd=sigma2d,
                      data.main=subset(mydata,mydata[,Iname]==0),
                      data.cali=subset(mydata,mydata[,Iname]==1))$x
    dhat=nleqslv(x=dhat,fn=Ud,b=bhat,sd=sigma2d,sb=sigma2b,
                 data.main=subset(mydata,mydata[,Iname]==0),
                 data.cali=subset(mydata,mydata[,Iname]==1))$x
    sigma2d=nleqslv(x=sigma2d,fn=Usd,b=bhat,d=dhat,sb=sigma2b,
                    data.main=subset(mydata,mydata[,Iname]==0),
                    data.cali=subset(mydata,mydata[,Iname]==1))$x
    par.vec=c(bhat,sigma2b,dhat,sigma2d)
    out[i,]=par.vec
  ...
  names(par_con)=c("b0","b1","b2",bW,"sb","d0","d1","d2",dZ,"sd")
  NIE.eee = par_mar[2] - par_con[2]
  MP.eee  =  (par_mar[2] - par_con[2])/par_mar[2]
  
  ### sandwich variance A^-1 B A^-1
  # (1) calculate B
  Ba=Ua(a=ahat,sa=sigma2a,c=chat,sc=sigma2c,data.main=subset(mydata,I==0),data.cali=subset(mydata,I==1),type=2)
  Bsa=Usa(sa=sigma2a,a=ahat,c=chat,sc=sigma2c,data.main=subset(mydata,I==0),data.cali=subset(mydata,I==1),type=2)
  Bc=Uc(c=chat,a=ahat,sa=sigma2a,sc=sigma2c,data.main=subset(mydata,I==0),data.cali=subset(mydata,I==1),type=2)
  Bsc=Usc(sc=sigma2c,a=ahat,c=chat,sa=sigma2a,data.main=subset(mydata,I==0),data.cali=subset(mydata,I==1),type=2)
  B1=rbind(Ba,Bsa,Bc,Bsc)
  Bb =Ub(b=bhat,sb=sigma2b,d=dhat,sd=sigma2d,data.main=subset(mydata,I==0),data.cali=subset(mydata,I==1),type=2)
  Bsb=Usb(sb=sigma2b,b=bhat,d=dhat,sd=sigma2d,data.main=subset(mydata,I==0),data.cali=subset(mydata,I==1),type=2)
  Bd =Ud(d=dhat,b=bhat,sb=sigma2b,sd=sigma2d,data.main=subset(mydata,I==0),data.cali=subset(mydata,I==1),type=2)
  Bsd=Usd(sd=sigma2d,b=bhat,d=dhat,sb=sigma2b,data.main=subset(mydata,I==0),data.cali=subset(mydata,I==1),type=2)
  B2=rbind(Bb,Bsb,Bd,Bsd)
  Bs=rbind(B1,B2)
  B=Bs %*% t(Bs)/dim(mydata)[1]
  # (2) calculate A
  Aa=gradU(name="Ua",a=ahat,sa=sigma2a,b=bhat,sb=sigma2b,c=chat,sc=sigma2c,d=dhat,sd=sigma2d,model=1)
  Asa=gradU(name="Usa",a=ahat,sa=sigma2a,b=bhat,sb=sigma2b,c=chat,sc=sigma2c,d=dhat,sd=sigma2d,model=1)
  Ac=gradU(name="Uc",a=ahat,sa=sigma2a,b=bhat,sb=sigma2b,c=chat,sc=sigma2c,d=dhat,sd=sigma2d,model=1)
  Asc=gradU(name="Usc",a=ahat,sa=sigma2a,b=bhat,sb=sigma2b,c=chat,sc=sigma2c,d=dhat,sd=sigma2d,model=1)
  Ab=gradU(name="Ub",a=ahat,sa=sigma2a,b=bhat,sb=sigma2b,c=chat,sc=sigma2c,d=dhat,sd=sigma2d,model=2)
  Asb=gradU(name="Usb",a=ahat,sa=sigma2a,b=bhat,sb=sigma2b,c=chat,sc=sigma2c,d=dhat,sd=sigma2d,model=2)
  Ad=gradU(name="Ud",a=ahat,sa=sigma2a,b=bhat,sb=sigma2b,c=chat,sc=sigma2c,d=dhat,sd=sigma2d,model=2)
  Asd=gradU(name="Usd",a=ahat,sa=sigma2a,b=bhat,sb=sigma2b,c=chat,sc=sigma2c,d=dhat,sd=sigma2d,model=2)
  A1=cbind(Aa,Asa,Ac,Asc)
  A2=cbind(Ab,Asb,Ad,Asd)
  A=rbind(cbind(t(A1), matrix(0, nrow=nrow(A1), ncol=ncol(A2))),
          cbind(matrix(0, nrow=nrow(A2), ncol=ncol(A1)), t(A2)))/dim(mydata)[1]
  Ai=solve(A)
  covP=(Ai %*% B %*% t(Ai))
  colnames(covP)=rownames(covP)=c("a0","a1",aW,"sa","c0","c1",cZ,"sc",
                                  "b0","b1","b2",bW,"sb","d0","d1","d2",dZ,"sd")
...
get_rc=function(data=mydata,Xname="X",Xsname="Xs",Yname="Y",Mname="M",Wname="W",Zname="W",Iname="I",outcome="binary") {
  ...
  if (outcome=="binary") {
    y.m1 = glm(fmar,family=binomial(link="logit"),data=subset(data,data[,Iname]==0))
    y.m2 = glm(fcon,family=binomial(link="logit"),data=subset(data,data[,Iname]==0))
  } else {
  ...
  if (outcome=="binary") {
    if (sum(subset(data,data[,Iname]==1)[,Yname])>0) {
      y.i1 = glm(fmarreal,family=binomial(link="logit"),data=subset(data,data[,Iname]==1))
      y.i2 = glm(fconreal,family=binomial(link="logit"),data=subset(data,data[,Iname]==1))
  ...
  Ua_f = function(a,data=mydata,outcome="binary") {
    ...
    if (outcome=="binary") {
      eps = data[,Yname] - expit(as.vector(cbind(1,data[,Xsname]) %*% a[1:2])+Wterm )
    } else {
  ...
  Ub_f = function(b,data=mydata,outcome="binary") {
    ...
    if (outcome=="binary") {
      eps = data[,Yname] - expit(as.vector(cbind(1,data[,Xsname],data[,Mname]) %*% b[1:3])+Wterm)
    } else {
...
get_naive=function(data=mydata,Xname="X",Xsname="Xs",Yname="Y",Mname="M",Wname="W",Zname="W",Iname="I",type="true",outcome="binary") {
  ...
  #data=mydata
  if (outcome=="binary") {
    y.m1 = glm(fmar,family=binomial(link="logit"),data=data)
    y.m2 = glm(fcon,family=binomial(link="logit"),data=data)
  } else {
...
#######################
set.seed(2022)
mydata=gen_data(n=1000,n2=200,rho=0.7,cormx=0.4,TE=log(1.5),MP=0.5,PY=0.5,outcome="continuous")
...
#    X           M        Y         Xs          W I
# 1 NA -0.07861102 1.822857 -0.2002656 -1.4038913 0
# 2 NA  0.22702352 2.654822  0.2734961  1.3763708 0
# 3 NA  0.41977096 1.398317  0.4318740  0.8339203 0
# 4 NA  0.27585670 1.546509  2.4026005  0.8805160 0
# 5 NA -0.39751675 1.794170 -0.2560545  0.6151019 0
# 6 NA  1.85265051 4.431562  3.3037772  2.4865786 0
...
get_rc(data=mydata,Xname="X",Xsname="Xs",Yname="Y",Mname="M",Wname="W",Zname="W",Iname="I",outcome="continuous")

################################
# Step 5: Naive Estimation
################################
get_naive(data=mydata,Xname="X",Xsname="Xs",Yname="Y",Mname="M",Wname="W",Zname="W",Iname="I",
          type="naive",outcome="continuous")
